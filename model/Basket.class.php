...
class Basket extends Model
{
    ...
    public static function check($id){
        $data = db::getInstance()->Select("SELECT * FROM cart WHERE id_good_catalog=$id");
    ...
    public static function remove($id){
        db::getInstance()->Query("DELETE FROM cart WHERE Id_good_cart=$id");
        $data = db::getInstance()->Select("SELECT cart.Id_good_cart, shop.id_good_catalog, makers.maker, shop.model, shop.price, shop.small_img, cart.quantity FROM cart NATURAL JOIN shop NATURAL JOIN makers");
        return $data;
    }
    public static function less($id){
        db::getInstance()->Query("UPDATE cart SET quantity=quantity-1  WHERE id_good_catalog=$id");
        $data = db::getInstance()->Select("SELECT  shop.price, cart.quantity FROM cart NATURAL JOIN shop NATURAL JOIN makers WHERE id_good_catalog=$id");
        return $data;
    }
    ...
    public function save(){
        $query = "INSERT INTO cart (id_good_catalog) VALUES (".$this->id_good.")";
        db::getInstance()->Query($query);
    }
}